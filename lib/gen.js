...
module.exports = () => {
    ...
    auth.provider = function useProvider(route, provider) {
        const mount = {}
        for(const method of ['get', 'post', 'put', 'delete', 'all']) {
            mount[method] = (sub, handler) => { auth[method]((route + sub).split('//').join('/'), handler) }
        }
        provider(mount)
    }
...
module.exports = function localProvider(mount) {
    mount.get('/me', (req, res) => {
        // send user info
        return res.json({ id: 0, name: 'John' })
    })
    mount.post('/register', (req, res) => {
        // register a user
    })
    mount.post('/login', (req, res) => {
        // login
    })
}`)
